...
**Plan Storage (For PLANNING Mode):** `.agent/plans/{task-slug}.md`

```bash
# Files go to .agent/plans/ folder
# File name based on task:
# "e-commerce site" → .agent/plans/ecommerce-site.md
# "add auth feature" → .agent/plans/auth-feature.md
```

> 🔴 **Location:** `.agent/plans/` folder - NOT project root or docs/ folder.
...
| 8 | **DYNAMIC NAMING** | `.agent/plans/{task-slug}.md` | Easy to find, multiple plans OK |
